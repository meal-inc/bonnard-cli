...
async function importConnections(connections: ParsedConnection[]): Promise<void> {
  ...
  if (imported > 0) {
    ...
    if (isSelfHosted(loadConfig())) {
      console.log();
      console.log(pc.yellow("Self-hosted: update .env with your database credentials, then restart Docker."));
      console.log(pc.dim("  See .bon/datasources.yaml for the imported connection details."));
    }
  }
  if (skipped > 0) {
...
async function addManual(options: AddOptions): Promise<void> {
  ...
  printSelfHostedHint(datasource);
...
/**
 * Map a datasource to CUBEJS_DB_* env vars for self-hosted .env configuration.
 */
function toCubeEnvVars(ds: LocalDatasource): Record<string, string> {
  const vars: Record<string, string> = {};

  // Map type to Cube driver name
  ...
  vars.CUBEJS_DB_TYPE = typeMap[ds.type] || ds.type;

  // Map config fields
  if (ds.config.host) vars.CUBEJS_DB_HOST = ds.config.host;
  if (ds.config.port) vars.CUBEJS_DB_PORT = ds.config.port;
  if (ds.config.database) vars.CUBEJS_DB_NAME = ds.config.database;
  if (ds.credentials.username) vars.CUBEJS_DB_USER = ds.credentials.username;
  if (ds.credentials.password) vars.CUBEJS_DB_PASS = ds.credentials.password;

  // SSL
  if (ds.config.sslmode === "require") vars.CUBEJS_DB_SSL = "true";

  // Snowflake-specific
  if (ds.config.account) vars.CUBEJS_DB_HOST = `${ds.config.account}.snowflakecomputing.com`;
  if (ds.config.warehouse) vars.CUBEJS_DB_SNOWFLAKE_WAREHOUSE = ds.config.warehouse;
  if (ds.config.role) vars.CUBEJS_DB_SNOWFLAKE_ROLE = ds.config.role;

  // BigQuery-specific
  if (ds.config.project_id) vars.CUBEJS_DB_BQ_PROJECT_ID = ds.config.project_id;

  // Databricks-specific
  if (ds.config.hostname) vars.CUBEJS_DB_DATABRICKS_URL = `https://${ds.config.hostname}`;
  if (ds.config.http_path) vars.CUBEJS_DB_EXPORT_BUCKET_MOUNT_DIR = ds.config.http_path;
  if (ds.credentials.token) vars.CUBEJS_DB_DATABRICKS_TOKEN = ds.credentials.token;

  // DuckDB-specific
  if (ds.config.database_path) vars.CUBEJS_DB_DUCKDB_DATABASE_PATH = ds.config.database_path;

  return vars;
}

/**
 * Print env var hint for self-hosted users after datasource add.
 */
function printSelfHostedHint(ds: LocalDatasource): void {
  if (!isSelfHosted(loadConfig())) return;

  const vars = toCubeEnvVars(ds);
  console.log();
  console.log(pc.yellow("Self-hosted: update .env with these values, then restart Docker:"));
  console.log();
