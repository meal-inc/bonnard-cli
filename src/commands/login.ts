...
async function loginWithToken(token: string) {
  try {
    ...
    if (!res.ok) {
      ...
      console.error(pc.red(`Token validation failed: ${message}`));
      process.exit(1);
    }

    const { email } = (await res.json()) as { email: string };
    saveCredentials({ token, email });
    console.log(pc.green(`Logged in as ${email}`));
  } catch (err) {
    console.error(pc.red(`Failed to validate token: ${err instanceof Error ? err.message : err}`));
    process.exit(1);
  }
}
