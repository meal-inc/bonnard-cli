...
export async function deployCommand(options: DeployOptions = {}) {
  ...
  // Step 2: Sync and test referenced datasources (cloud only)
  const { loadConfig, isSelfHosted, getBaseUrl } = await import("../lib/config.js");
  const config = loadConfig();
  const selfHosted = isSelfHosted(config);

  // Message is required for cloud deploys (deployment history), optional for self-hosted
  if (!selfHosted && !options.message) {
    console.log(pc.red("Deploy message required. Use -m \"your message\" to describe your changes."));
    process.exit(1);
  }
  ...
  try {
    ...
    if (selfHosted) {
      console.log(pc.dim(`  ${response.deployment.fileCount} files deployed`));
      console.log();
      console.log(pc.bold("Connect AI agents via MCP:"));
      const mcpUrl = `${getBaseUrl(config)}/mcp`;
