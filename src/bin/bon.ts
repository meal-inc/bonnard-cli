...
program
  .command("init")
  .description("Create bon.yaml, bonnard/cubes/, bonnard/views/, .bon/, and agent templates")
  .option("--update", "Update agent templates to match installed CLI version")
  .option("--self-hosted", "Generate Docker Compose infrastructure for self-hosted mode")
  .option("--agent <name>", "Coding agent to generate templates for (claude, cursor, codex, windsurf, copilot, cline)")
  .action(initCommand);

program
  .command("login")
  .description("Authenticate with Bonnard via your browser")
  .option("--token <token>", "Authenticate with a Clerk API key (for Docker/CI)")
  .action(loginCommand);
...
program
  ...
  .option("-m, --message <text>", "Deploy message describing your changes")
