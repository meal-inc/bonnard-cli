...
### Added
- `bon login --token <key>` — authenticate with a Clerk API key directly, for Docker containers, CI/CD, and headless environments
- `BON_TOKEN` environment variable — skip `bon login` entirely by setting the token in the environment
...
### Changed
- `bon init --self-hosted` no longer scaffolds `DEPLOY.md` — deployment docs now in the [public repo README](https://github.com/bonnard-data/bonnard)
